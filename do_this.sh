...
if ! command -v appstreamcli &> /dev/null; then
    ...
    report P "${GRN}AppStream install verified."; sleep 2
fi
...
if [ "$AUTO" = "TRUE" ]; then
    ...
    if flatpak list | grep -q "$AFPBASE"; then
        ...
        report N "${WHT}Previous version not found. Installing new version."
    fi
else
    report N "${WHT}Auto mode isn't active - you'll have to manually uninstall and reinstall Amulet Flatpak Edition."
    lastWord
fi
...
if [ "$DEBUG" = "TRUE" ]; then
    ...
    clear
    echo -e "\n${YLW}Once inside, type '${RED}python -vvv -m pdb -m amulet_map_editor${YLW}' to run Amulet though ${WHT}PDB${YLW}."; sleep 2
