...
modules:
  ...
  - name: launcher-script
    ...
    build-commands:
      ...
      - echo 'cd /app && python -vv -m amulet_map_editor \$1' >> /app/bin/amulet
...
while [[ "$1" != "" ]]; do
    case "$1" in
        ...
        --yaml=*)
            DO_THIS_YAML="${1#*=}"
            report N "${WHITE}Using '${DO_THIS_YAML}' to build."
            shift
            ;;
        --command=*)
            DO_CMD="${1#*=}"
            report N "${WHITE}Will run '${DO_CMD}' in flatpak instead of 'amulet'."
            shift
            ;;
        --debug)
            DEBUG=true
...
if [ "$DEBUG" = "true" ]; then
    ...
    #clear
    report N "${WHITE}Auto-Mode and DEBUG are active." #; sleep 2
    ...
    if ! flatpak run --ostree-verbose -vv --command=${DO_CMD} --devel --filesystem="$(pwd)" $AFPBASE; then
        report F "Amulet Flatpak debug execution failed."; bye $LINENO
    fi
else
    ...
    report N "${WHITE}Auto-Mode active. Running: ${YELLOW}flatpak run --ostree-verbose -vv --command=${DO_CMD} --filesystem=\"\$(pwd)\" ${AFPBASE}${RESET}" #; sleep 2
    if ! flatpak run --ostree-verbose -vv --command=${DO_CMD} --filesystem="$(pwd)" $AFPBASE; then
        report F "${RED}Amulet launch failed. Please review terminal output."
        bye $LINENO
    fi
fi

report P "Looks like it probably worked. If it didn't, then something is ${RED}REALLY${WHITE} messed up, and I need to go back to the drawing board!"
lastWord
