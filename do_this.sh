...
if [ "$DEBUG" = "TRUE" ]; then
    ...
    if ! flatpak install --include-sdk --include-debug -vvv -y --user amulet-x86_64.flatpak; then
        ...
        report N "${WHT}Configuring Debug extension ($AFPDBG)"; sleep 2
        if ! flatpak install --user -y ./$AFPREPO $AFPDBG
            report F "$AFPDBG failed"; echo -e "${WHT}"
            read "Try to continue without $AFPDBG (y/n)? " tryCont
            case $tryCont in
                n) exit 1
                ;;
            esac
    ...
    echo -e "\n${WHT}flatpak run --command=sh --devel --filesystem=$(pwd) $AFPBASE $AFP_YML"
    if ! flatpak run --command=sh --devel --filesystem=$(pwd) $AFPBASE $AFP_YML; then
        report F "Amulet Flatpak install failed."
        exit 1
    fi
else
